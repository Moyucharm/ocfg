...
本项目是一个独立的 OpenCode 配置文件编辑器。此前计划的 OpenCode 插件包装器已在 v1 中弃用，也不会作为受支持的工作流进行宣传。
...
该工具编辑 OpenCode 配置文件，不会修改 OpenCode 核心。

OpenCode 插件包装器已在 v1 中弃用。

不承诺运行时提供商热重载。如果正在运行的 OpenCode 会话没有获取到提供商变更，请重启 OpenCode。

该工具不会替代 OpenCode 内置的模型选择器。

该工具在 v1 中不支持 OpenCode Web UI。
