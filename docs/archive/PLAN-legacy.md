...
## Wave 8: Plugin Wrapper (Deprecated)

Status: deprecated. The standalone CLI/TUI already edits OpenCode config directly, so a plugin wrapper that only logs or hints at the CLI does not provide enough user value for v1. Do not implement this wave unless a future OpenCode plugin API offers a concrete capability that the standalone tool cannot provide safely.
...
<task type="auto" id="8.1">
  ...
  <instructions>
    Deprecated: do not execute for v1.
