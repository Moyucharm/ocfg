...
import { useTuiLanguage, useTuiText } from "../i18n.js"
  ...
  useEffect(() => {
    ...
    async function inspect() {
      ...
      try {
        ...
        if (!active) return
        setDiagnostics(runDoctor(document, { language }))
      } catch (caught) {
        if (!active) return
      ...
      {severities.map((severity) => {
        ...
        return (
          <Box key={severity} flexDirection="column" marginTop={severity === "high" ? 0 : 1}>
            <Box paddingX={5}>
              <Text bold color={theme.colors.section}>{severity === "high" ? t("doctor.severity.high") : severity === "medium" ? t("doctor.severity.medium") : t("doctor.severity.low")}</Text>
            </Box>
